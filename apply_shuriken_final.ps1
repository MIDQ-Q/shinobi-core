...
Write-Host "=== SHURIKEN + MARK + IDLE (FINAL) ===" -ForegroundColor Cyan

function PatchN($file, $marker, $insert, $name, $sentinel) {
    ...
    if ($cn.Contains($mn)) {
        ...
        Write-Host "[SKIP] $name (marker not found!)" -ForegroundColor Red
    }
}

# ================= НОВЫЕ ФАЙЛЫ =================
[System.IO.File]::WriteAllText("$src\combat\MarkTracker.java", @'
...
'@, $utf8); Write-Host "[F] MarkTracker.java" -ForegroundColor Green

[System.IO.File]::WriteAllText("$src\combat\ThrowingHelper.java", @'
package com.example.shinobicore.combat;
...
'@, $utf8); Write-Host "[F] ThrowingHelper.java" -ForegroundColor Green

[System.IO.File]::WriteAllText("$src\entity\ShurikenEntity.java", @'
...
'@, $utf8); Write-Host "[F] ShurikenEntity.java" -ForegroundColor Green

[System.IO.File]::WriteAllText("$src\entity\ShurikenRenderer.java", @'
package com.example.shinobicore.entity;
...
'@, $utf8); Write-Host "[F] ShurikenRenderer.java" -ForegroundColor Green

[System.IO.File]::WriteAllText("$src\item\ThrowingWeaponItem.java", @'
...
'@, $utf8); Write-Host "[F] ThrowingWeaponItem.java" -ForegroundColor Green

[System.IO.File]::WriteAllText("$src\item\ModItems.java", @'
package com.example.shinobicore.item;
...
'@, $utf8); Write-Host "[F] ModItems.java" -ForegroundColor Green

[System.IO.File]::WriteAllText("$src\client\IdlePoseSystem.java", @'
...
'@, $utf8); Write-Host "[F] IdlePoseSystem.java" -ForegroundColor Green

# ================= ПАТЧИ =================
PatchN "$src\entity\ModEntities.java" 'public static void register() {' @'
public static final EntityType<ShurikenEntity> SHURIKEN = Registry.register(
    ...
    public static void register() {
'@ "ModEntities: SHURIKEN" "SHURIKEN"

PatchN "$src\ShinobiCore.java" 'ModEntities.register();' @'
ModEntities.register();
        ModItems.register();
'@ "ShinobiCore: ModItems.register()" "ModItems.register()"

PatchN "$src\ShinobiCore.java" 'import com.example.shinobicore.entity.ModEntities;' @'
import com.example.shinobicore.entity.ModEntities;
import com.example.shinobicore.item.ModItems;
'@ "ShinobiCore: ModItems import" "import com.example.shinobicore.item.ModItems;"

PatchN "$src\client\ShinobiCoreClient.java" 'EntityRendererRegistry.register(ModEntities.NINJA_PROJECTILE, NinjaProjectileRenderer::new);' @'
...
'@ "ShinobiCoreClient: shuriken renderer" "ShurikenRenderer"

PatchN "$src\client\ShinobiCoreClient.java" 'import com.example.shinobicore.entity.NinjaProjectileRenderer;' @'
import com.example.shinobicore.entity.NinjaProjectileRenderer;
import com.example.shinobicore.entity.ShurikenRenderer;
'@ "ShinobiCoreClient: renderer import" "import com.example.shinobicore.entity.ShurikenRenderer;"

PatchN "$src\client\ShinobiCoreClient.java" 'ChakraHudRenderer.exhausted = packet.exhausted();' @'
ChakraHudRenderer.exhausted = packet.exhausted();
                ClientNinjaState.meditating = packet.meditating();
'@ "ShinobiCoreClient: meditating sync" "ClientNinjaState.meditating"

PatchN "$src\client\ClientNinjaState.java" 'public static boolean sensoryEnabled = true;' @'
public static boolean sensoryEnabled = true;
    public static boolean meditating = false;
'@ "ClientNinjaState: meditating" "meditating"

PatchN "$src\client\ClientNinjaState.java" 'public static boolean dangerSense = false;' @'
public static boolean dangerSense = false;
    public static boolean meditating = false;
'@ "ClientNinjaState: meditating (alt)" "meditating"

PatchN "$src\client\ChakraPhysicsClient.java" 'private static boolean wasStickingToWall = false;' @'
private static boolean wasStickingToWall = false;
    public static boolean stickingToWall = false;
'@ "ChakraPhysicsClient: stickingToWall" "stickingToWall = false"

PatchN "$src\client\ChakraPhysicsClient.java" 'wasStickingToWall = stickingNow;' @'
wasStickingToWall = stickingNow;
            stickingToWall = stickingNow;
'@ "ChakraPhysicsClient: update flag" "stickingToWall = stickingNow"

PatchN "$src\mixin\PlayerRenderAnimationMixin.java" @'
...
'@ "Mixin: idle hook" "IdlePoseSystem.apply"
...
'@ "Mixin: IdlePoseSystem import" "import com.example.shinobicore.client.IdlePoseSystem;"

PatchN "$src\jutsu\AoeBehavior.java" 'living.damage(player.getDamageSources().magic(), damage);' 'living.damage(player.getDamageSources().magic(), MarkTracker.boost(living, damage));' "AoeBehavior: mark boost" "MarkTracker"
PatchN "$src\jutsu\AoeBehavior.java" 'import com.example.shinobicore.stat.NinjaPlayerData;' @'
import com.example.shinobicore.stat.NinjaPlayerData;
import com.example.shinobicore.combat.MarkTracker;
'@ "AoeBehavior: import" "import com.example.shinobicore.combat.MarkTracker;"

PatchN "$src\jutsu\MeleeBehavior.java" 'target.damage(player.getDamageSources().magic(), damage);' 'target.damage(player.getDamageSources().magic(), MarkTracker.boost(target, damage));' "MeleeBehavior: mark boost" "MarkTracker"
PatchN "$src\jutsu\MeleeBehavior.java" 'import com.example.shinobicore.stat.NinjaPlayerData;' @'
import com.example.shinobicore.stat.NinjaPlayerData;
import com.example.shinobicore.combat.MarkTracker;
'@ "MeleeBehavior: import" "import com.example.shinobicore.combat.MarkTracker;"

PatchN "$src\jutsu\DashBehavior.java" 'target.damage(player.getDamageSources().magic(), damage);' 'target.damage(player.getDamageSources().magic(), MarkTracker.boost(target, damage));' "DashBehavior: mark boost" "MarkTracker"
PatchN "$src\jutsu\DashBehavior.java" 'import com.example.shinobicore.stat.NinjaPlayerData;' @'
import com.example.shinobicore.stat.NinjaPlayerData;
import com.example.shinobicore.combat.MarkTracker;
'@ "DashBehavior: import" "import com.example.shinobicore.combat.MarkTracker;"

PatchN "$src\entity\NinjaProjectileEntity.java" 'hitEntity.damage(this.getDamageSources().magic(), damage);' 'hitEntity.damage(this.getDamageSources().magic(), MarkTracker.boost(hitEntity, damage));' "NinjaProjectileEntity: mark boost" "MarkTracker"
PatchN "$src\entity\NinjaProjectileEntity.java" 'import com.example.shinobicore.ShinobiCore;' @'
import com.example.shinobicore.ShinobiCore;
import com.example.shinobicore.combat.MarkTracker;
'@ "NinjaProjectileEntity: import" "import com.example.shinobicore.combat.MarkTracker;"

PatchN "$res\data\shinobicore\skill_tree\tree.json" '    "kekkei":    {"angle": 0,   "color": "#FF66CC", "label": "Kekkei Genkai", "hidden": true},' @'
    "shuriken":  {"angle": 0,   "color": "#BBBBBB", "label": "Shurikenjutsu"},
    "kekkei":    {"angle": 0,   "color": "#FF66CC", "label": "Kekkei Genkai", "hidden": true},
'@ "tree.json: shuriken branch" '"shuriken"'

PatchN "$res\data\shinobicore\skill_tree\tree.json" @'
  ]
}
'@ @'
...
'@ "tree.json: shuriken nodes" "shuriken_accuracy"

PatchN "$src\client\SkillTreeScreen.java" '"sensory", "space", "kekkei"' '"sensory", "space", "shuriken", "kekkei"' "SkillTreeScreen: shuriken column" '"shuriken"'
...
if (-not (Test-Path $dir)) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }
[System.IO.File]::WriteAllText("$dir\shuriken.json", '{ "parent": "item/generated", "textures": { "layer0": "shinobicore:item/shuriken" } }', $utf8)
[System.IO.File]::WriteAllText("$dir\kunai.json", '{ "parent": "item/generated", "textures": { "layer0": "shinobicore:item/kunai" } }', $utf8)
...
Write-Host "[F] recipes/models/lang" -ForegroundColor Green

# Текстуры (если ещё нет)
$texDir = "$res\assets\shinobicore\textures\item"
if (-not (Test-Path "$texDir\shuriken.png")) {
    try {
        ...
        $pts = @([System.Drawing.Point]::new(8, 0), [System.Drawing.Point]::new(10, 6),
        ...
        $bmp.Save("$texDir\shuriken.png", [System.Drawing.Imaging.ImageFormat]::Png)
        ...
        $bladePts = @([System.Drawing.Point]::new(13, 0), [System.Drawing.Point]::new(16, 5),
            [System.Drawing.Point]::new(9, 9), [System.Drawing.Point]::new(7, 7))
        $g.FillPolygon($blade, $bladePts)
        $g.FillRectangle($handle, 4, 8, 4, 6)
        $g.FillEllipse($handle, 2, 13, 4, 3)
        $bmp.Save("$texDir\kunai.png", [System.Drawing.Imaging.ImageFormat]::Png)
        $g.Dispose(); $bmp.Dispose()
        Write-Host "[F] textures generated" -ForegroundColor Green
    } catch {
        Write-Host "[SKIP] textures: $($_.Exception.Message)" -ForegroundColor Yellow
    }
} else {
    Write-Host "[OK] textures already exist" -ForegroundColor Gray
}
...
Write-Host "  /give @p shinobicore:shuriken 16" -ForegroundColor White
Write-Host "  /give @p shinobicore:kunai 16" -ForegroundColor White
Write-Host "  ПКМ - бросок (aim-assist от perception), попадание = метка (контур +20% урона техник)" -ForegroundColor White
Write-Host "  Стой на месте: дыхание; L (чакра) = стойка ниндзя; M (удерж.) = поза медитации" -ForegroundColor White