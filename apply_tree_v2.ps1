# ============================================================
# SHINOBI CORE — SKILL TREE v2 (Pufferfish's Skills Style)
# ============================================================
...
Write-Host "=== SKILL TREE v2 ===" -ForegroundColor Cyan

# === [0] БЭКАПЫ ===
$backupDir = "$root\backup_tree_v2"
if (-not (Test-Path $backupDir)) { New-Item -ItemType Directory -Path $backupDir -Force | Out-Null }
foreach ($f in @("SkillTreeNode.java","SkillTreeRegistry.java","SkillTreeScreen.java",
    "ProgressionScreen.java","KeyBindings.java","ClientInputHandler.java","ShinobiCore.java")) {
    $path = Get-ChildItem -Path $src -Recurse -Filter $f | Select-Object -First 1
    if ($path) { Copy-Item $path.FullName "$backupDir\$f" -Force }
}
Write-Host "[0] Backups saved" -ForegroundColor Gray
...
public class SkillTreeScreen extends Screen {
    ...
    private static final int BG_GRADIENT_TOP = 0xFF0A0A1A;
    private static final int BG_GRADIENT_BOT = 0xFF04040A;
    ...
    @Override public void render(DrawContext ctx, int mx, int my, float delta) {
        ...
        drawCentered(ctx, "LMB: unlock  |  RMB drag: pan  |  Scroll: (future zoom)",
    ...
    private boolean isBranchVisible(BranchDef b) {
        ...
        if (b.hidden()) {
            // Скрытая ветка: видна если хотя бы 1 узел на ней видим
...
# === [5] ProgressionScreen.java — вкладка Tree ===
$file = "$src\client\ProgressionScreen.java"
$content = [System.IO.File]::ReadAllText($file, $utf8)

# Меняем tab 4 (Tree) — клик открывает SkillTreeScreen
$marker = 'if (tab == 4) {
...
# Проверим есть ли уже этот блок
if (-not $content.Contains('new SkillTreeScreen()')) {
    # Ищем блок tab == 4 в mouseClicked
    $tabMarker = 'if (tab == 4) {'
    if ($content.Contains($tabMarker)) {
        # Найдём весь блок if (tab == 4) { ... }
        # Простая замена: ищем комментарий про Древо
        $oldBlock = @'
        if (tab == 4) {
            // Древо: клик открывает полный экран
            if (this.client != null) {
...
'@
        ...
        # Нужно вставить блок tab == 4 в mouseClicked
        # Ищем блок tab == 3 и добавляем tab == 4 после него
        $insertAfter = '            return true;
        }

        // Прокачка и аттюнмент'
        $newBlock = @'
            return true;
        }
        ...
        // Прокачка и аттюнмент
'@
        $content = $content.Replace($insertAfter, $newBlock)
    }
}

# Проверим импорт
if (-not $content.Contains('import com.example.shinobicore.client.SkillTreeScreen;')) {
    # Найдём первый import и добавим
    $firstImport = 'import com.example.shinobicore.client.attunement.AttunementScreen;'
    if ($content.Contains($firstImport)) {
        $content = $content.Replace($firstImport, $firstImport + "`nimport com.example.shinobicore.client.SkillTreeScreen;")
    }
}

[System.IO.File]::WriteAllText($file, $content, $utf8)
Write-Host "[5] ProgressionScreen.java" -ForegroundColor Green

# === [6] KeyBindings.java — убираем SKILL_TREE ===
$file = "$src\client\KeyBindings.java"
$content = [System.IO.File]::ReadAllText($file, $utf8)

# Удаляем объявление
$content = $content -replace '(?m)^\s*public static KeyBinding SKILL_TREE;\s*$', ''

# Удаляем регистрацию (многострочную)
$content = $content -replace '(?s)\s*SKILL_TREE = KeyBindingHelper\.registerKeyBinding\(new KeyBinding\(\s*"key\.shinobicore\.skill_tree"[^;]*?\)\);', ''

[System.IO.File]::WriteAllText($file, $content, $utf8)
Write-Host "[6] KeyBindings.java" -ForegroundColor Green

# === [7] ClientInputHandler.java — убираем J ===
$file = "$src\client\ClientInputHandler.java"
$content = [System.IO.File]::ReadAllText($file, $utf8)

# Удаляем блок SKILL_TREE
$content = $content -replace '(?s)\s*// === ДРЕВО ПРОКАЧКИ \(J\) ===\s*if \(KeyBindings\.SKILL_TREE\.wasPressed\(\)\) \{[^}]+\}', ''

# Удаляем импорт
$content = $content -replace '(?m)^import com\.example\.shinobicore\.client\.SkillTreeScreen;\s*$', ''

[System.IO.File]::WriteAllText($file, $content, $utf8)
Write-Host "[7] ClientInputHandler.java" -ForegroundColor Green

# === [8] ShinobiCore.java — проверка видимости при unlock ===
$file = "$src\ShinobiCore.java"
$content = [System.IO.File]::ReadAllText($file, $utf8)

# В handleUnlockNode добавляем проверку видимости
$marker = '        if (data.isNodeUnlocked(nodeId)) {
...
$insert = @'
        ...
        // Проверка видимости (клан + условия)
...
[System.IO.File]::WriteAllText($file, $content, $utf8)
Write-Host "[8] ShinobiCore.java" -ForegroundColor Green
