...
Write-Host "=== TREE v3: ADVANCEMENT STYLE ===" -ForegroundColor Cyan

# === [1] SkillTreeScreen.java — ПОЛНАЯ ПЕРЕДЕЛКА ===
...
public class SkillTreeScreen extends Screen {
    ...
    private float dragViewX, dragViewY;
    ...
    private int[] worldPos(SkillTreeNode n) {
        return new int[]{ colX(n.branch()) + (int)(n.angleOffset() * 2), TOP + n.distance() * ROW_H };
    }
    ...
    public void render(DrawContext ctx, int mx, int my, float delta) {
        ...
        // === КОННЕКТОРЫ (угловатые, как в достижениях) ===
    ...
    // === СПИРАЛЬ УЗУМАКИ (фоновая печать) ===
    ...
    // === ТУЛТИП-СВИТОК (пергамент, вайб Наруто) ===
...
Write-Host "[1] SkillTreeScreen.java: advancement-style columns" -ForegroundColor Green

# === [2] tree.json — медицинская ветка отдельно ===
$file = "$res\data\shinobicore\skill_tree\tree.json"
$j = [System.IO.File]::ReadAllText($file, $utf8)

# Добавляем ветку medical после taijutsu
$marker = '"taijutsu":  {"angle": 309, "color": "#66FF66", "label": "Taijutsu"},'
$insert = @'
...
# Переносим мед-узлы в свою колонку
$j = $j.Replace('"id":"med_palm","branch":"general","distance":2,"angleOffset":12,',
                '"id":"med_palm","branch":"medical","distance":1,')
$j = $j.Replace('"id":"med_poison","branch":"general","distance":3,"angleOffset":12,',
                '"id":"med_poison","branch":"medical","distance":2,')

[System.IO.File]::WriteAllText($file, $j, $utf8)
Write-Host "[2] tree.json: medical column added" -ForegroundColor Green

Write-Host "`n=== TREE v3 COMPLETE ===" -ForegroundColor Cyan
