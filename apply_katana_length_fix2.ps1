# ============================================================
#  FIX2: KATANA BLADE +33% AND THIRD-PERSON GRIP FIX
#  Run: powershell -ExecutionPolicy Bypass -File .\apply_katana_length_fix2.ps1
# ============================================================
...
Write-Host "`n=== KATANA FIX2: blade +33%, thirdperson grip fix ===" -ForegroundColor Cyan

# ================================================================
# 1. REGENERATE KATANA MODELS (longer blade + fixed 3rd person)
# ================================================================
function El($f, $t, $px) {
    $uv = @($px, 0, ($px + 1), 1)
    $faces = @{}
...
function New-KatanaModelJson($tex) {
    ...
    # handle (unchanged - grip stays in hand)
    ...
    # blade: 7.3 -> 25.6 (was 21.0, +33%)
    [void]$els.Add((El @(7.6,7.3,7.8)  @(8.4,25.6,8.2)  0))  # blade
    [void]$els.Add((El @(7.6,7.3,7.68) @(8.4,25.6,7.8)  1))  # edge (light)
    [void]$els.Add((El @(7.6,7.3,8.2)  @(8.4,25.6,8.32) 2))  # spine (dark)
    [void]$els.Add((El @(7.75,25.6,7.85) @(8.25,27.0,8.15) 1)) # kissaki
    $model = [ordered]@{
        gui_light = "front"
        ...
        display = [ordered]@{
            thirdperson_righthand = @{ rotation = @(0,-90,20); translation = @(0,3.5,0.5); scale = @(0.85,0.85,0.85) }
            thirdperson_lefthand  = @{ rotation = @(0,90,-20); translation = @(0,3.5,0.5); scale = @(0.85,0.85,0.85) }
            ...
            fixed  = @{ rotation = @(0,0,45); translation = @(0,0,0); scale = @(0.4,0.4,0.4) }
        }
    }
...
# ================================================================
# 2. BACK SCABBARD: extend to match longer blade
# ================================================================
...
Write-Host "Tuning: if 3rd person tilt is too much/little, edit Z value (20) in thirdperson rotation of katana_*.json" -ForegroundColor DarkGray