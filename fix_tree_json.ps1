...
# Бэкап повреждённого
if (Test-Path $file) { Copy-Item $file "E:\Games\mod\backup_phase_a\tree.json.broken" -Force }
...
Write-Host "tree.json rewritten" -ForegroundColor Green

# === ВАЛИДАЦИЯ JSON ===
try {
    $parsed = Get-Content $file -Raw -Encoding UTF8 | ConvertFrom-Json
    $nodeCount = $parsed.nodes.Count
    $branchCount = $parsed.branches.PSObject.Properties.Count
    Write-Host "JSON VALID: $nodeCount nodes, $branchCount branches" -ForegroundColor Green
} catch {
