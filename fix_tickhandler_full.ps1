...
Write-Host "[OK] NinjaTickHandler.java rewritten from scratch" -ForegroundColor Green

# Проверка баланса скобок
$openCount = ($code.ToCharArray() | Where-Object {$_ -eq '{'}).Count
$closeCount = ($code.ToCharArray() | Where-Object {$_ -eq '}'}).Count
Write-Host "Braces: open=$openCount, close=$closeCount"
if ($openCount -eq $closeCount) {
    Write-Host "[OK] Balanced" -ForegroundColor Green
} else {
    Write-Host "[FAIL] Unbalanced!" -ForegroundColor Red
}
